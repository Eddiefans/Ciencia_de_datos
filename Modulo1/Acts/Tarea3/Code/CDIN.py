...
class CDIN:
    ...
    def get_binarios(df):
        ...
        for col in column_cat:
            if df_cat[col].nunique()==2:
                columns_binarias.append(col)
        return columns_binarias, df[columns_binarias]
    
    def get_categoricos(df):
        df_categorico = df.select_dtypes(include = "object")
        columns_categoricas = df_categorico.columns
        return columns_categoricas, df_categorico
    ...
    def replace_text(x,to_replace, replacement):
        try:
            x = x.replace(to_replace,replacement, inplace = True)
        except:
            pass
        return x
    @staticmethod
